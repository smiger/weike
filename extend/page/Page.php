...
class Page extends Paginator
{
    ...
    protected function info(){
        return "<p class='pageRemark'>" . $this->pageSizeSelect() . "共<b>" . $this->lastPage .
            "</b>页<b>" . $this->total . "</b>条数据</p>";
    }

    private function pageSizeSelect() {
        $pageSize = request()->param('pageSize', $this->listRows);

        return "每页显示" . 
            "<select onchange='pageSizeSelect(this)'>" .
            "<option" . ($pageSize == 15 ? " selected" : "") . ">15</option>" .
            "<option" . ($pageSize == 50 ? " selected" : "") . ">50</option>" .
            "<option" . ($pageSize == 100 ? " selected" : "") . ">100</option>" .
            "<option" . ($pageSize == 150 ? " selected" : "") . ">150</option>" .
            "</select>" .
            "条&nbsp;&nbsp;";
    }
     ...
     * @return string
     */
    protected function pageSizeUrl($page)
    {
        $query = $this->options['query'];

        if (isset($this->options['query']['pageSize'])) {
            unset($this->options['query']['pageSize']);
        }
        $url = $this->url($page);
    ...
    protected function cssAndJs(){
        ...
        function pageSizeSelect(obj) {
            var pageSize = $(obj).val();
            window.location.href = "' . $this->pageSizeUrl(1) . '&pageSize=" + pageSize;
        }
        </script>';
    }
}
